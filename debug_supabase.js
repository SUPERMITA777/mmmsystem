...
const URL = 'https://xnupjsxbvyirpeagbloe.supabase.co/rest/v1/config_impresion?select=*&limit=1';
...
async function debug() {
    try {
        ...
        if (data && data[0]) {
            console.log('Columns in config_impresion:', Object.keys(data[0]).join(', '));
            console.log('Sample row:', JSON.stringify(data[0], null, 2));
        } else {
            console.log('No data found in config_impresion');
        }
    } catch (e) {
