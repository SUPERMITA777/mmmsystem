...
interface ProductosSortModalProps {
    isOpen: boolean;
    onClose: () => void;
    productos: Producto[];
    onSaved: () => void;
...
export default function ProductosSortModal({
    isOpen,
    onClose,
    productos: initialProductos,
    onSaved,
}: ProductosSortModalProps) {
    const [items, setItems] = useState(initialProductos);
    const [isSaving, setIsSaving] = useState(false);

    useEffect(() => {
        if (isOpen) {
            setItems(initialProductos);
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-start">
                    <div>
                        <h2 className="text-xl font-bold text-slate-900">Ordenar Productos</h2>
                        <p className="text-sm text-slate-500 mt-1">Arrastrá los productos para cambiar su orden.</p>
                    </div>
                    <button
                        onClick={onClose}
